#!/usr/bin/env bash
# Module 5 · Spikee — iterative jailbreak (single-turn LLM) (complete)
#
# Runs spikee's 'llm_jailbreaker' attack against Larkfield. objective = cross-customer PII (same as every method).
...
#   bash <course>/modules/module5/spikee/iterative/complete.sh
#
set -euo pipefail
...
spikee generate --seed-folder "$SEEDS" --format user-input --include-standalone-inputs --tag m5iterativec
DS=$(ls -t datasets/*m5iterativec*.jsonl | head -1)
