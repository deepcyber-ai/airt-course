...
COST: PAIR refines in parallel (no branching, no on-topic check) — PyRIT builds it on the
same TAP machinery, but the two are NOT identical: TAP explores breadth, PAIR depth. Still
many calls; watch 'target requests this run'.

    export AIRT_ATTACKER=qwen            # or AIRT_GROUP=openai (multi-turn only)
    python3 modules/module5/pyrit/pair/complete.py
"""
import asyncio
import sys
...
async def main() -> None:
    ...
    save_trace(target, "pair-complete")
