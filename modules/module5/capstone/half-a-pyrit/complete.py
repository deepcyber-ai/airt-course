"""Module 5 capstone · Half A — enhanced PyRIT attack (complete).

You drive a named multi-turn technique with a LARGER budget than the Module 5
examples. This worked version runs PAIR with a bigger tree (width 4, depth 6) at
the shared capstone objective (cross-customer PII). No cap — watch
'target requests this run'; a bigger budget means more requests and more cost.

    export AIRT_ATTACKER=gpt-4o-mini    # SAME attack model as Half B (for a fair compare)
    python3 modules/module5/capstone/half-a-pyrit/complete.py
"""
import asyncio
import sys
from pathlib import Path

# The shared pyrit helpers (_common.py) live in the sibling pyrit module — this
# resolves it in both the source (labs/pyrit) and export (modules/module5/pyrit) layouts.
...
async def main() -> None:
    ...
    attack = PAIRAttack(
        ...
        tree_width=4, tree_depth=6)     # ENHANCED budget (Module 5 default was 3 x 4)
    result = await attack.execute_async(objective=OBJECTIVE)
    show(result, target, EXPECTED_FLAG)
    save_trace(target, "capstone-halfA-pair")
