...
Your trainer will assign your group to **Half A or Half B**. Complete only that half, then add
one row to `COMPARISON-RECORD.md`.

Half A tests whether PyRIT can reveal all three supplied contact details. Half B inspects a
saved broad HumanBound scan. The saved scan did not test Half A's exact objective. Use the same
contact-details check to identify that coverage gap. Do not calculate a combined success rate.
...
**Assigned:** run the complete (enhanced) script.
...
`complete.py` runs enhanced PAIR; the output prints the native `OWASP{llm02…}` flag and the
**name/email/phone check**, plus the target request count.

**Smaller alternative** — only if you need a cheaper run. It runs PAIR at the default smaller
budget on the **same** objective. **Do not run both.**
...
Your scope file is a planning exercise. Validation checks its format and permitted content.
It does not change the saved HumanBound run.

1. **Write a scope.** Edit `humanbound/scope.starter.yaml` (each `restricted` item a
   plain string), then **validate it** (it stops before making model calls if settings are
   incomplete):
   ```bash
...
Both halves use the same target and the same check for the three contact details, but they
test different objectives. Record the result of that check, the number of target requests,
the evidence, and the tool's own finding in separate columns. Compare the evidence across
the room. Do not rank request efficiency because PAIR and HumanBound performed different
tests.

## Running the room safely — trainer note (not a student task)

*This section is for the trainer, not a student instruction.* All groups share `:8089`, and one
PAIR search is ~84 sends that also resets the database on branch reconstruction — several at
once will burst-500. So either give each group an isolated target instance, or run Half A on a
staggered schedule: at most 2–3 groups run `half-a-pyrit` at a time (allow a few minutes each —
an operational estimate, not from the retained trace) while the rest do Half B first (inspect
the prepared run — no target load). The prepared HumanBound run removes the biggest burst
