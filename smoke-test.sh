#!/usr/bin/env bash
# AMLUCS course pack — Module 5-7 lab smoke test.
#
...
# (M6 scorer, M7 attack/score, M5 single-attacker) and the Promptfoo lab, and prints
# PASS / FAIL / SKIP per check. It changes nothing and pushes nothing. A live run costs
# a few US cents of model calls; the M6 offline check costs nothing.
#
...
# 1. M6 offline — no key, no target: proves the lab imports and runs
...
# 2. M6 live — marker vs model scorer over the bundled examples
...
# 3. M7 attack + score (needs the target)
...
# 4. M5 single-turn attacker + scorer
step "Module 5 — pyrit/single-attacker/complete.py"
if [ "$TARGET_UP" = 1 ]; then
...
# 5. Promptfoo (Node tool — needs the key in the shell)
step "Module 5 — promptfoo/single-attacker (Node)"
...
if [ "$TARGET_UP" != 1 ]; then sk "promptfoo — target :8089 down"
  ...
  if [ -z "${OPENAI_API_KEY:-}" ]; then sk "promptfoo — OPENAI_API_KEY not in shell (source /opt/airt/src/.env)"
    ...
    if printf '%s' "$pf_out" | grep -qE "[0-9]+ passed|[0-9]+ failed"; then ok "promptfoo"
